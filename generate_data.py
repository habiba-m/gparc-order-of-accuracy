...
def gaussian_exact(X, Y, t, x0, y0, sigma):
    ...
    xc = (x0 + VX * t) % L
    yc = (y0 + VY * t) % L
    dx = X - xc;  dx = dx - L * np.round(dx / L)
    dy = Y - yc;  dy = dy - L * np.round(dy / L)
...
def _rk4(u_hat, dt, kx, ky):
    k1 = _rhs(u_hat,             kx, ky)
    k2 = _rhs(u_hat + .5*dt*k1, kx, ky)
    k3 = _rhs(u_hat + .5*dt*k2, kx, ky)
...
def make_edge_index(N):
    ...
    return torch.tensor(
        np.stack([np.concatenate(rows), np.concatenate(cols)]),
        dtype=torch.long
    )
...
def sample_gaussian_ics(n):
    ...
    return [dict(type='gaussian', x0=float(x0[i]), y0=float(y0[i]),
                 sigma=float(sigma[i])) for i in range(n)]
...
def sample_disc_ics(n):
    ...
    return [dict(type='disc', x0=float(x0[i]), y0=float(y0[i]),
                 radius=float(radius[i])) for i in range(n)]
...
def build_sim(N, ic, include_exact=False):
    ...
    pos        = torch.from_numpy(
        np.stack([(X / L).flatten(), (Y / L).flatten()], axis=1).astype(np.float32)
    )                                               # [N², 2]
    ...
    if ic_type == 'gaussian':
        ...
        fields = [
            disc_exact(X, Y, t, ic['x0'], ic['y0'], ic['radius']).flatten()
            for t in t_snap
        ]
    else:
        ...
        if include_exact:
            ...
            if ic_type == 'gaussian':
                exact = gaussian_exact(
                    X, Y, t_snap[ti + 1], ic['x0'], ic['y0'], ic['sigma']
                ).flatten()
            else:
                exact = disc_exact(
                    X, Y, t_snap[ti + 1], ic['x0'], ic['y0'], ic['radius']
                ).flatten()
...
def generate_split(problem, split, ics, N=TRAIN_N):
    ...
    for i, ic in enumerate(ics):
        ...
        meta = {**ic, 'split': split, 'N': N, 'vx': VX, 'vy': VY,
                'n_steps': len(sim)}
        save_sim(out_dir / f"sim_{i:04d}.pt", sim, meta)
        ic_str = (f"x0={ic['x0']:.2f} y0={ic['y0']:.2f} "
                  f"σ={ic['sigma']:.2f}" if ic['type'] == 'gaussian'
                  else f"x0={ic['x0']:.2f} y0={ic['y0']:.2f} r={ic['radius']:.2f}")
...
def generate_eval(problem, ics, eval_ns=EVAL_NS):
    ...
    for N in eval_ns:
        ...
        for i, ic in enumerate(ics):
            ...
            meta = {**ic, 'split': 'eval', 'N': N, 'h': float(L / N),
                    'vx': VX, 'vy': VY, 'n_steps': len(sim)}
            save_sim(out_dir / f"sim_{i:04d}.pt", sim, meta)
